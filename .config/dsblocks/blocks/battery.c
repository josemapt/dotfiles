...
#define ICONe                           COL2 "E" COL0 /* unexpected error */
#define ICONa                           COL1 "N" COL0 /* no battery */
...
#define BATP                            20 /* plug in level */
#define BATU                            90 /* unplug level */

#define BATCAPFILE                      "/sys/class/power_supply/BAT0/capacity"
#define ACSTATEFILE                     "/sys/class/power_supply/ADP1/online"
...
battery(char *str, int ac)
{
        ...
        if (ac == NILL) {
                ...
                if (ac) {
                        ...
                        snprintf(str, BLOCKLENGTH, COL2 "%s%d%%", ICON(bat), bat);
                } else {
                        if (bat > BATP)
                                level = Normal;
                        else if (bat > BATL) {
